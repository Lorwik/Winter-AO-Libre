    ...
    If NpcIndex > 0 Then
        ...
        If UserList(UserIndex).Invent.WeaponEqpObjIndex = EspadaMataDragonesIndex Then ' Usa la matadragones?
            ModifClase = ModicadorDañoClaseArmas(UserList(UserIndex).Clase)
                ...
                DañoArma = RandomNumber(Arma.MinHIT * 3, Arma.MaxHIT * 2)
...
If Not UserList(UserIndex).flags.Privilegios = PlayerType.User Then Exit Function
...
Call SendData(ToAll, 0, 0, "||El clan " & Guilds(UserList(UserIndex).GuildIndex).GuildName & " esta atacando al rey del castillo Oeste" & FONTTYPE_FENIX)
...
Else
    ...
    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "||" & vbRed & "°Fallas°" & str(UserList(UserIndex).Char.CharIndex))
End If
...
If UserList(AtacanteIndex).flags.Invisible = 1 Then
   ...
   Call SendData(SendTarget.ToIndex, AtacanteIndex, 0, "PRB39")
End If
...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "PRE51," & ExpaDar)
    End If
