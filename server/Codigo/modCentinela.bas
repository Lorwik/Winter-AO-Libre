...
Public NPC_CENTINELA_TIERRA As Byte  'Índice del NPC en el .dat
Public NPC_CENTINELA_AGUA As Byte       'Ídem anterior, pero en mapas de agua
 
Public CentinelaNPCIndex As Integer                'Índice del NPC en el servidor
 
Public TIEMPO_INICIAL As Byte  'Tiempo inicial en minutos. No reducir sin antes revisar el timer que maneja estos datos.
 
Private Type tCentinela
    RevisandoUserIndex As Integer   '¿Qué índice revisamos?
    TiempoRestante As Byte      '¿Cuántos minutos le quedan al usuario?
...
On Error GoTo Error_Handler
    ...
    If Not UserList(Centinela.RevisandoUserIndex).flags.CentinelaOK Then
        ...
        Call SendData(SendTarget.ToAdmins, 0, 0, "||Servidor> El centinela ha encarcelado a " & name & FONTTYPE_SERVER)
        'ponemos el flag de ban a 1
        Call Encarcelar(name, 5)
        'ponemos la pena
        numPenas = val(GetVar(CharPath & name & ".chr", "PENAS", "Cant"))
        Call WriteVar(CharPath & name & ".chr", "PENAS", "Cant", numPenas + 1)
        Call WriteVar(CharPath & name & ".chr", "PENAS", "P" & numPenas + 1, "CENTINELA : Carcel POR MACRO INASISTIDO " & Date & " " & Time)
