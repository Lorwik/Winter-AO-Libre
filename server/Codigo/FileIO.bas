...
For Hechizo = 1 To NumeroHechizos
    ...
    Hechizos(Hechizo).ArmorBreak = val(Leer.GetValue("Hechizo" & Hechizo, "ArmorBreak"))
    ...
    For LoopC = 1 To LastUser
        If UserList(LoopC).GuildIndex <> 0 Then
            ...
            If Guilds(UserList(LoopC).GuildIndex).GuildName = CastilloOESTE Then
                UserList(LoopC).Stats.GLD = UserList(LoopC).Stats.GLD + 100000
                Call SendData(SendTarget.Toindex, (LoopC), 0, "||Has Recibido 100.000 de oro por mantener el castillo Oeste" & FONTTYPE_FENIX)
                Call SendUserStatsBox(LoopC)
                Call SendData(SendTarget.Toindex, 0, 0, "TW")
            End If
...
For Object = 1 To NumObjDatas
    ...
    ObjData(Object).NoSubasta = val(Leer.GetValue("OBJ" & Object, "NoSubasta")) 'Subastas de EAOExtraido por lorwik
    ObjData(Object).NoRoba = val(Leer.GetValue("OBJ" & Object, "NoRoba"))
    ...
    Dim a As Integer
    For a = 1 To NUMRAZAS
        ObjData(Object).RazaProhibida(a) = Leer.GetValue("OBJ" & Object, "RP" & a)
    Next a
...
UserList(userindex).Stats.Masdos = CLng(UserFile.GetValue("STATS", "Masdos"))
...
OESTE = val(GetVar(App.Path & "\Castillos.ini", "CASTILLO", "OESTE"))
...
Ramx.Map = GetVar(DatPath & "Ciudades.dat", "Ramx", "Mapa")
Ramx.x = GetVar(DatPath & "Ciudades.dat", "Ramx", "X")
Ramx.Y = GetVar(DatPath & "Ciudades.dat", "Ramx", "Y")
...
#End If
    ...
    NPC_CENTINELA_TIERRA = GetVar(IniPath & "Server.ini", "CENTINELA", "CentTierra")
 
    NPC_CENTINELA_AGUA = GetVar(IniPath & "Server.ini", "CENTINELA", "CentAgua")
   
    TIEMPO_INICIAL = GetVar(IniPath & "Server.ini", "CENTINELA", "Tiempo")
   
    centinelaActivado = GetVar(IniPath & "Server.ini", "CENTINELA", "Activado")
End Sub
...
Call WriteVar(UserFile, "STATS", "Masdos", CStr(UserList(userindex).Stats.Masdos))
...
Npclist(NpcIndex).GiveEXP = val(GetVar(npcfile, "NPC" & NpcNumber, "GiveEXP")) * EXPI


Npclist(NpcIndex).GiveGLD = val(GetVar(npcfile, "NPC" & NpcNumber, "GiveGLD")) * OROI
